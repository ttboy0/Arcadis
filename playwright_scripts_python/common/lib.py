...
class PlaywrightManager:
    ...
    def validate_element_from_data(self, yaml_file_path: str) -> bool:
        ...
        try:
            # First, try to open the mobile menu to make navigation elements visible
            try:
                mobile_menu_button = self.page.locator('button[aria-label="Open Mobile Menu"]').first
                if mobile_menu_button.is_visible():
                    print("Opening mobile menu to access navigation elements...")
                    mobile_menu_button.click()
                    self.page.wait_for_timeout(1000)  # Wait for menu to open
            except Exception as e:
                print(f"Note: Could not open mobile menu: {str(e)}")
