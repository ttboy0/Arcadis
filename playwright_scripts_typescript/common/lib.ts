...
export class PlaywrightManager {
    ...
    async validateElementFromData(yamlFilePath: string): Promise<boolean> {
        ...
        try {
            ...
            try {
                const mobileMenuButton = this.page.locator('button[aria-label="Open Mobile Menu"]').first();
                const isVisible = await mobileMenuButton.isVisible();
                if (isVisible) {
                    console.log("Opening mobile menu to access navigation elements...");
                    await mobileMenuButton.click();
                    await this.page.waitForTimeout(1000); // Wait for menu to open
                }
            } catch (error) {
                console.log(`Note: Could not open mobile menu: ${error}`);
            }
